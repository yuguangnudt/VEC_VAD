...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--number_workers', '-nw', '--num_workers', type=int, default=4)
    parser.add_argument(
        '--number_gpus',
        '-ng',
        type=int,
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if osp.exists(args.validation_dataset_root):
            ...
            block.log('Validation Input: {}'.format(' '.join([
                str([d for d in x.size()]) for x in validation_dataset[0][0]
            ])))
            block.log('Validation Targets: {}'.format(' '.join([
                str([d for d in x.size()]) for x in validation_dataset[0][1]
            ])))
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            progress.set_description(
                ...
                tools.format_dictionary_of_losses(
                    loss_labels, np.array(statistics).mean(axis=0)))
            progress.update(1)
    ...
    best_err = 1e8
    progress = tqdm(
        range(args.start_epoch, args.total_epochs + 1),
        miniters=1,
        ncols=100,
